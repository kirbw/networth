import { ensureChartJs, chartPalette } from "./charts.js";
import { PAGE_META } from "./page-meta.js";
import { decoratePageShell, updateHomeKpis } from "./shell.js";
import { syncThemePreference } from "./theme.js";
...
function renderAuthState() {
  ...
  if (sessionName) {
    ...
    sessionName.textContent = authenticated ? (currentUser?.fullName || currentUser?.username || "User") : "Guest";
  ...
  const topbar = document.querySelector(".topbar");
  if (topbar) topbar.setAttribute("aria-hidden", authenticated ? "false" : "true");
  syncThemePreference(currentUser);
}
...
async function handleLogin(event) {
  ...
  if (page === "home" && document.body.classList.contains("public-home")) {
...
function initHomePage() {
  ...
  async function renderInvestmentsSummary() {
    ...
    const entries = [
      { label: "Stocks", value: summary.stocks || 0, color: palette.accent },
      { label: "Precious Metals", value: summary.preciousMetals || 0, color: palette.positive },
      { label: "Real Estate", value: summary.realEstateMyValue || 0, color: palette.violet },
      { label: "Business Ventures", value: summary.businessVenturesMyValue || 0, color: palette.warning },
      { label: "Retirement Accounts", value: summary.retirementAccounts || 0, color: palette.accentAlt },
    ...
    summaryState.investmentsTotal = Number(summary.combinedTotal || 0);
    if (investmentsTotalEl) investmentsTotalEl.textContent = `Combined total: ${currency(summaryState.investmentsTotal)}`;
    renderHomeKpis();
  }
  ...
  async function renderLiabilitiesSummary() {
    ...
    const summary = await response.json();
    const entries = [
      { label: "Mortgages", value: summary.mortgages || 0, color: palette.danger },
      { label: "Credit Cards", value: summary.creditCards || 0, color: palette.warning },
      { label: "Loans", value: summary.loans || 0, color: palette.muted },
    ].sort((a, b) => b.value - a.value);
    ...
    summaryState.liabilitiesTotal = Number(summary.combinedTotal || 0);
    if (liabilitiesTotalEl) liabilitiesTotalEl.textContent = `Combined liabilities: ${currency(summaryState.liabilitiesTotal)}`;
    renderHomeKpis();
  }
  ...
  async function renderAssetsSummary() {
    ...
    const entries = [
      { label: "Bank Accounts", value: bankAccounts.reduce((sum, x) => sum + Number(x.balance || 0), 0), color: palette.accentAlt },
      { label: "Vehicles", value: vehicles.reduce((sum, x) => sum + Number(x.value || 0), 0), color: palette.accent },
      { label: "Equipment", value: equipment.reduce((sum, x) => sum + Number(x.value || 0), 0), color: palette.violet },
      { label: "Guns", value: guns.reduce((sum, x) => sum + Number(x.value || 0), 0), color: palette.muted },
      { label: "Cash", value: cash.reduce((sum, x) => sum + Number(x.amount || 0), 0), color: palette.positive },
    ].sort((a, b) => b.value - a.value);
    ...
    if (assetsTotalEl) {
      ...
      summaryState.assetsTotal = combined;
  ...
  function renderHomeKpis() {
    ...
    const latestNetWorthRecord = [...records].reverse().find((record) => record.netWorth != null) || null;
    const cumulativeIncome = records.reduce((sum, record) => sum + Number(record.income || 0), 0);
    const cumulativeGiving = records.reduce((sum, record) => sum + Number(record.donation || 0), 0);
    const givingRate = cumulativeIncome > 0 ? cumulativeGiving / cumulativeIncome : 0;
    updateHomeKpis({
      latestIncome: latestRecord ? currency(latestRecord.income || 0) : "$0",
      incomeNote: latestRecord ? `Most recent tax year: ${latestRecord.year}` : "Add your first annual record",
      latestNetWorth: latestNetWorthRecord ? currency(latestNetWorthRecord.netWorth || 0) : "$0",
      netWorthNote: latestNetWorthRecord ? `Most recent net worth year: ${latestNetWorthRecord.year}` : "Add net worth to your annual data",
      givingRate: percent(givingRate),
      givingNote: `${currency(cumulativeGiving)} donated across ${records.length || 0} saved years`,
      balanceSheet: currency(summaryState.assetsTotal + summaryState.investmentsTotal - summaryState.liabilitiesTotal),
      balanceNote: `${currency(summaryState.assetsTotal + summaryState.investmentsTotal)} tracked assets and investments`,
    });
  }
  ...
  async function renderCharts() {
    ...
    incomeGivingChart = new Chart(document.getElementById("income-giving-chart").getContext("2d"), {
      type: "line",
      data: { labels: records.map((r) => r.year), datasets: [{ label: "Income", data: records.map((r) => r.income), borderColor: palette.accent, backgroundColor: palette.accent, tension: 0.2 }, { label: "Donations", data: records.map((r) => r.donation), borderColor: palette.positive, backgroundColor: palette.positive, tension: 0.2 }] },
    ...
    netWorthChart = new Chart(document.getElementById("net-worth-chart").getContext("2d"), {
      type: "line",
      data: { labels: nw.map((r) => r.year), datasets: [{ label: "Net Worth", data: nw.map((r) => r.netWorth), borderColor: palette.violet, backgroundColor: palette.violet, tension: 0.2 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom" } }, scales: { y: { ticks: { callback: (v) => formatCompactCurrency(v) } } } },
    });
    ...
    goalProgressChart = new Chart(document.getElementById("goal-progress-chart").getContext("2d"), {
      type: "line",
      data: { labels: records.map((r) => r.year), datasets: [{ label: "Cumulative Income", data: cumIncomeSeries, borderColor: palette.accent, backgroundColor: palette.accent }, { label: "Cumulative Donations", data: cumDonationSeries, borderColor: palette.positive, backgroundColor: palette.positive }, { label: `${goalPercent.toFixed(1)}% Target`, data: targetSeries, borderColor: palette.warning, backgroundColor: palette.warning, borderDash: [6, 6], pointRadius: 0 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom" } }, scales: { y: { ticks: { callback: (v) => formatCompactCurrency(v) } } } },
    });
    renderHomeKpis();
  }
  ...
  return { render: async () => { await renderCharts(); await renderInvestmentsSummary(); await renderLiabilitiesSummary(); await renderAssetsSummary(); } };
}
...
function initProfilePage() {
  ...
  const themePreferenceInput = document.getElementById("profile-theme-preference");
  ...
  async function render() {
    ...
    if (themePreferenceInput) themePreferenceInput.value = currentUser.themePreference || (currentUser.darkMode ? "dark" : "light");
  ...
  settingsForm?.addEventListener("submit", async (event) => {
    ...
    const themePreference = themePreferenceInput?.value || (darkModeInput?.checked ? "dark" : "light");
    const notifications = {
    ...
    const response = await apiFetch("/api/user-settings", { method: "POST", body: JSON.stringify({ givingGoalPercent, themePreference, notifications }) });
    const data = await response.json();
  ...
  async function renderChart() {
    ...
    hunterChart = new Chart(chartCanvas, {
      ...
      data: {
        ...
        datasets: [{ label: "Deer Harvested", data: Object.values(counts), backgroundColor: palette.accent }],
      },
...
function initInvestmentCalculatorReportPage() {
  ...
  async function renderProjection(points) {
    if (!chartEl) return;
    await ensureChartJs();
    const palette = chartPalette();
    ...
    investmentProjectionChart = new Chart(chartEl.getContext("2d"), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: `${palette.accent}29`,
          borderWidth: 3,
          fill: true,
          tension: 0.25,
          pointRadius: 2,
